...
| D5 | Selector | `#launcher-<id> #launcher-arrow`, generated only by `lc_id_selector_new()` | `#launcher-<id>` alone; `#launcher-button` | Inner button paints and hovers; its effective name is `launcher-arrow` (research lane 3, three tags). `#launcher-button` matches nothing and fails silently |
...
| D7 | Provider priority | **Single decision point**: `LC_PROVIDER_PRIORITY` + `lc_provider_attach()` in `src/glue/lc-provider.c`, driven by meson option `-Dprovider_priority=application\|user`. No other file may name a priority constant or an attach function | Priority literal at each call site | Open question 1 is empirical (H0-b); it must be a one-line change. `lc_provider_attach()` also owns screen-level vs widget-level scope, because widget-level beats screen-level at equal priority |
...
**Parsing.** The strict parser becomes record-oriented rather than line-oriented, which is affordable because we are the only writer and the layout is fixed: match `/* id=<digits> fp=<1–16 hex> */`, then the selector line, then declarations until a closing `}`. Any record that does not match this shape in full is skipped, exactly as an unparsable line was before. A rule whose marker is **absent or malformed** is not skipped — the rule is still read and the entry is kept with an empty fingerprint, which D15 heals at the next save. Nothing has shipped, so there is no migration to write; this rule exists because a truncated or hand-edited file must not cost the user their colours. The banner carries `schema=2` to distinguish the shape from the marker-free draft.
...
| `src/core/lc-id.{c,h}` | Create | `lc_id_is_valid`, `lc_id_parse`, `lc_id_selector_new` (D5) |
| `src/core/lc-color.{c,h}` | Create | `LcColor {r,g,b,a}`, `lc_color_parse`, `lc_color_to_css`, `lc_color_to_hex` |
| `src/core/lc-css.{c,h}` | Create | `lc_css_rule_new`, `lc_css_document_new`, `LcStyle` |
...
gchar    *lc_id_selector_new       (gint id);                 /* "#launcher-13 #launcher-arrow" */
...
- [ ] **Provider priority** — `APPLICATION` (600) vs `USER` (800), given the panel's widget-level `APPLICATION` provider on `plugin->button`. Resolved empirically in H0-b. Isolated to `LC_PROVIDER_PRIORITY` + `lc_provider_attach()` in `src/glue/lc-provider.c` and the `-Dprovider_priority` meson option; no other file may name a priority constant.
- [ ] **Does the background visibly paint** over Adwaita and the Xubuntu default theme without breaking hover? H0-b. Same decision point also owns screen-level vs widget-level attach scope.
