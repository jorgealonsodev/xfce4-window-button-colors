...
| D5 | Selector | `#launcher-<id> #launcher-arrow`, base selector generated only by `lc_id_selector_new(id)`; the state pseudo-class is appended to the **last** element by `lc_css_rule_new()`, the single producer (D18, amended) | `#launcher-<id>` alone; `#launcher-button`; assembling selectors by concatenation at call sites | Inner button paints and hovers; its effective name is `launcher-arrow` (research lane 3, three tags). `#launcher-button` matches nothing and fails silently. GTK3 permits a state pseudo-class only on the last element of a chain, so `#launcher-<id>:hover #launcher-arrow` is the same silent-no-match trap — see D18 |
...
| D18 | State pseudo-class placement | The pseudo-class goes on the **last** element only: `#launcher-<id> #launcher-arrow:hover`. `lc_css_rule_new()` is the sole producer and appends it to the trailing element by construction (**amended**: originally `lc_id_selector_new(id, state)`; moved because D17 requires all three rules from one call and `lc-css` stays id-agnostic — residual `const gchar *` gap recorded in the D18 section) | Let call sites concatenate `":hover"` where convenient | GTK3 permits state pseudo-classes only on the final element of a selector chain. `#launcher-<id>:hover #launcher-arrow` matches nothing and fails **silently** — the identical failure shape to the `#launcher-button` trap that D5 exists to prevent. Structural prevention rather than a comment, for the same reason as D16 |
...
**Amended after slice 1/3a implementation — the placement guarantee stands, its enforcement point moved.**

```c
gchar *lc_id_selector_new (gint id);                                        /* base selector only */
gchar *lc_css_rule_new (const gchar *selector, const LcColor *, const LcStyle *);  /* appends the pseudo-classes */
```

GTK3 permits a state pseudo-class only on the last element of a selector chain, so `#launcher-13 #launcher-arrow:hover` is correct and `#launcher-13:hover #launcher-arrow` matches nothing and fails silently — the same trap as `#launcher-button`. That requirement is unchanged and is what this decision exists to protect.

What changed is which function appends. The original shape put the state on `lc_id_selector_new(id, state)`. As built, `lc_id_selector_new()` produces the base selector only and `lc_css_rule_new()` appends `:hover` and `:active` to the trailing element. The reason is D17: all three rules must be emitted by **one** call, because geometry and `background-image: none` belong on the base rule alone. Threading `LcState` through the selector producer would force either three selectors into `lc_css_rule_new()` or a dependency from `lc-css` onto `lc-id`'s id semantics, and `lc-css` is deliberately id-agnostic — it takes a selector string.

The single-producer property is preserved: exactly one function in the tree concatenates a pseudo-class, and it appends to the end of whatever selector it is given, so a selector produced by `lc_id_selector_new()` can only ever come out correct.

**Residual gap, recorded rather than hidden.** `lc_css_rule_new()` takes a `const gchar *`, so a call site *could* hand it a hand-built `"#launcher-13:hover #launcher-arrow"` and it would compile. This is the same weakness D16 closed for documents with an opaque `LcDocument`, and D16's own text rejects a `const char *` parameter for exactly this reason. It is not closed here today; the guarantee currently rests on single-producer discipline plus the `test-id.c` regression asserting `#launcher-button` is never emitted. Closing it structurally means an opaque `LcSelector` whose sole constructor is `lc_id_selector_new()` — deferred, not decided against.
